...
export const ContextPanel: React.FC = () => {
  ...
  const gitPaneWidth = React.useMemo(() => {
    ...
    const gitModeWidthFraction = panelState?.widthFractionByMode?.['git'];
    const effectiveGitModeWidth = gitModeWidthFraction != null && availablePanelAreaWidth != null
      ? Math.round(gitModeWidthFraction * availablePanelAreaWidth)
      : gitModeWidth;
    return clampWidth(
      effectiveGitModeWidth ?? Math.round(getContextSurfaceWidthFraction('git') * widthFallbackBase),
      maxPanelWidth(availablePanelAreaWidth ?? widthFallbackBase),
    );
  }, [availablePanelAreaWidth, panelState?.widthByMode, panelState?.widthFractionByMode, widthFallbackBase]);
  ...
  const clampWidthForDrag = React.useCallback((nextWidth: number) => {
    ...
    if (isSplitMode) {
      // The split total is bounded by the available area only; the panel has
      // no fixed pixel ceiling on wide monitors.
  ...
  }, [activeTab?.mode, innerDiffTabs.length, isTreeOnly]);
  ...
  }, [activeModeForWidth, activeTab?.mode, clampWidthForDrag, directoryKey, gitPaneWidth, innerDiffTabs.length, isTreeOnly, setContextEditorTreeWidth, setContextGitSplitDiffWidth, setContextPanelWidth, splitTotalWidth, width]);
