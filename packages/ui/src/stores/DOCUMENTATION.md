...
Input history keeps both runtime-wide and runtime/directory/session buckets in one bounded browser-storage envelope. Every scope change, append, and session cleanup rereads the latest durable envelope before applying its mutation, so a stale tab preserves history written by another tab. The in-memory copy is a fallback only when browser storage is unavailable or cannot be read.
