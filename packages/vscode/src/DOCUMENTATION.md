...
- `managed-opencode-process.ts` and `opencode.ts`
  - Default local connections now use the CLI-owned shared service. Start discovers or starts that service; stop disconnects the extension and clears its connection credentials. Restart reconnects without stopping shared execution. Explicit external URLs keep their existing connection policy. The extension does not publish OpenChamber web/desktop tool callbacks.
  - The owned-process helper described below remains separate from shared-service connections; a shared service is never registered for extension-owned termination or orphan reaping.
