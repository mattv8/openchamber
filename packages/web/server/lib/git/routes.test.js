...
describe('git commit file routes', () => {
  ...
  it('keeps the legacy commit preview query contract', async () => {
    gitLibraries.getCommitFileDiff.mockResolvedValue({ original: 'before', modified: 'after' });
    const { app, getRoute } = createRouteRegistry();
    registerGitRoutes(app);
    const response = createMockResponse();
    ...
    expect(response.statusCode).toBe(200);
    expect(gitLibraries.getCommitFileDiff).toHaveBeenCalledWith('/repo', 'd'.repeat(40), 'file.ts', true);
  });

  it('reports an invalid root comparison as a client error', async () => {
    gitLibraries.getCommitFiles.mockRejectedValue(new Error(`parentHash is required for non-root commit ${'e'.repeat(40)}`));
    const { app, getRoute } = createRouteRegistry();
    registerGitRoutes(app);
    const response = createMockResponse();
